...
重要规则：
- 即时约束（如「我想听播客」）的优先级高于用户长期偏好
- 本维度只评价关键因素的遵守情况，不评价操作效率

如果 context 中没有标注关键因素，给 8 分（中性高分）。""" + ANTI_LENIENCY_PROMPT
...
注意：如果关键因素（如「儿童上车」）导致操作偏离用户日常偏好，这是合理的。""" + ANTI_LENIENCY_PROMPT
...
重要：5-6 分是最需要仔细把关的区间。
如果你无法指出这个操作方案为什么特别适合这个场景，就不应该超过 6 分。""" + ANTI_LENIENCY_PROMPT
